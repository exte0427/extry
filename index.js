...
function makecode(evals,eva,iss,html,x,y,color,tocolor){
  ...
  sosso.push(iss);
  if(evals!="}"){sdasdasdadsad(codes,iss);}
  xs.push(0);
  ys.push(0);
  if(evals!="while"){
    is.push(-1);
  ...
  if(evals=="while"){
    codes++;
    makecode('}','},1);',0,4,82,y+32,'#FFBF00','#d8a200');
  }
  else{
    codes++;
  }
...
codegen("while","setInterval(function() {",0,`while true`,"#FFBF00","#d8a200");//#FFBF00
codegen("del","}",0,`//end`,"#FFBF00","#d8a200");//#FFBF00
